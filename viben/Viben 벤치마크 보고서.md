...
<details>
<summary>📊 상세 점수표 보기 (클릭하여 펼치기)</summary>

| **Category** | **Item** | **VibeX** | **Bolt** | **Lovable** | **Replit** | **Manus** | **v0** |
| :--- | :--- | :---: | :---: | :---: | :---: | :---: | :---: |
| **A. Core (60%)** | 의도 파악 (Context) | 3.9 | **5.0** | 4.4 | 4.8 | 4.8 | 3.9 |
| | 이미지 생성 (Multimodal) | **4.1** | 4.0 | 3.6 | 3.9 | 3.8 | 3.4 |
| | 로컬 언어 최적화 (Localization) | **5.0** | 3.9 | 3.5 | 3.1 | 3.0 | 3.0 |
| | 기능 완결성 (Logic) | 4.0 | **4.1** | 4.0 | 4.0 | 4.0 | 3.9 |
| | UI 구조 (Layout) | 4.0 | 4.0 | 4.0 | **4.1** | 4.0 | 4.0 |
| | 생성 깊이 (Depth) | 4.0 | **5.0** | 4.1 | 4.0 | 4.0 | 3.5 |
| | **A_w (가중평균)** | 4.092 | **4.392** | 4.007 | 4.020 | 3.978 | 3.696 |
| **B. Dev (30%)** | 코드 품질 (Quality) | 4.0 | 4.0 | 4.1 | 4.2 | **4.5** | 4.0 |
| | 생성 효율 (Efficiency) | **5.0** | 4.0 | **5.0** | 4.0 | 3.4 | **5.0** |
| | 비용 효율 (Cost) | **4.0** | 3.9 | 3.5 | 3.9 | 3.5 | 3.4 |
| | 오류 처리 (Healing) | 4.0 | 3.5 | 4.0 | 3.9 | **4.5** | 4.0 |
| | **B_w (가중평균)** | **4.400** | 3.946 | 4.388 | 4.076 | 3.932 | 4.326 |
| **C. Brand (10%)** | 브랜드 시스템 (Identity) | 4.0 | 3.9 | 4.2 | 4.1 | **4.5** | 4.0 |
| | UI 일관성 (Consistency) | 4.9 | 4.2 | 4.6 | 4.8 | **5.0** | 4.9 |
| | 에셋 통일감 (Cohesion) | **4.9** | 4.1 | 4.8 | 4.6 | 4.8 | 4.6 |
| | **C_w (가중평균)** | 4.528 | 4.069 | 4.505 | 4.466 | **4.737** | 4.463 |
| **Total** | Raw Score | **84.6** | 84.5 | 83.4 | 81.6 | 80.8 | 79.2 |
| | **Final Score** | **86.5** | **86.5** | 85.5 | 83.9 | 83.2 | 81.8 |
...
3. **시각 검증을 통한 로컬화 격차**
    ...
    - Playwright 및 텍스트 분석 결과, **VibeX**는 렌더링 및 현지 포맷 준수 여부(**Localization**)에서 높은 점수를 기록함.
...
- **VibeX (86.5점)** 는 **[기획 → 디자인 → 개발 → 마케팅(로고 생성)]의 Full-Cycle 역량**에서 강점을 보임. 이미지 생성의 강점, 브랜드 아이덴티티와 높은 현지화를 통합 지원하여, 개발 직후 즉시 시장 출시가 필요한 비즈니스 프로젝트에 최적화된 솔루션으로 평가됨
...
#### 2. Final Score (최종 점수)
- 정의: Raw Score를 기반으로 최소 65점 ~ 최대 100점 범위로 재조정한 보정 점수.
- 의미: 점수가 너무 낮게 나오거나(과락 방지), 특정 구간에 몰리는 것을 방지하여 **일반적인 성적표(65~100점)** 처럼 직관적으로 이해할 수 있도록 변환한 값. <br>
- 순위 결정: 최종 리포트의 순위는 이 Final Score를 기준으로 매겨짐.
- 계산식: <br>

$$Final Score = \max(65, 65 + (Raw Score - 60) \times \frac{35}{40})$$
